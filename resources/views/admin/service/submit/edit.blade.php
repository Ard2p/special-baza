    ...
    <div class="panel">
        <div class="panel-body">
            <ol class="breadcrumb">
                ...
                <li><a href="{{route('submit-service.index')}}"> Все заяки</a></li>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="col-md-6">
                        ...
                        @if($form->contractor_service)
                            <div class="form-group">
                                <label>Услуга Пользователя</label>
                                <input class="form-control" value="{{$form->contractor_service->name}}" type="text"
                                       disabled>
                            </div>
                        @endif
                        @if($form->service)
                            <div class="form-group">
                                <label>Услуга</label>
                                <input class="form-control" value="{{$form->service->title}}" type="text"
                                       disabled>
                            </div>
                        @endif
                        <div class="form-group">
                        ...
                        <div class="form-group input-group">
                            ...
                            <div class="input-group-btn">
                                ...
                                <a target="_blank" class="btn btn-default" href="{!!  $form->url !!}"
                                   style="    margin-top: 25px;">Перейти</a>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="btn-group">
                        ...
                        <div class="form-group">
                            <label class="checkbox-inline">
                                <input type="checkbox" value="1"
                                       {{($form->user->email_confirm) ? 'checked' : ''}} disabled>Подтвержден Email
                            </label>
                            <label class="checkbox-inline">
                                <input type="checkbox" value="1"
                                       {{$form->user->phone_confirm ? 'checked' : ''}} disabled>Подтвержден Телефон
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <form class="row" method="POST" id="create_proposal"
                  action="{{route('submit-service.update', $form->id)}}">
                @csrf
                @method('PATCH')
                ...
                <div class="col-md-12">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group form-element-dependentselect">
                            ...
                            <select style="width:100%;" class="form-control input-select column-filter"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}" {{$form->type_id === $category->id ? 'selected' : ''}}>{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group form-element-dependentselect">
                            ...
                            <select style="width:100%;"
                                ...
                                @foreach($regions as $region)
                                    <option value="{{$region->id}}" {{$form->region_id === $region->id ? 'selected' : ''}}>{{$region->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <label>Адрес</label>
                            <textarea class="form-control" type="text" name="address">{{$form->comment}}</textarea>
                        </div>
                    </div>
