    ...
    <div class="panel">
        <div class="panel-body">
            <form class="row" id="user_filter">
                ...
                <div class="col-md-12">
                    <h3>Создать динамический список {{$request->type === 'phone' ? 'Телефон' : 'Email'}}</h3>
                    <div class="col-md-6">
...
<script>
    document.addEventListener("DOMContentLoaded", function (event) {
        ...
        $(document).on('click', '#save_filtration', function (e) {
            e.preventDefault()
            var result = prompt('Введите название фильтра.');
            if (result == null) {
                return;
            }
            var url = $(this).data('url')
            var $form = $('#user_filter')
            $.ajax({
                url: url,
                type: 'POST',
                data: $form.serialize() + '&name=' + result,
                success: function (e) {
                    swal(e.message)
                    $('[name=filter_id]').html(e.data)
                },
                error: function (e) {
