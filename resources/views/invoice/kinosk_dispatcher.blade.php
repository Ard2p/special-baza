    ...
    <tr>
        ...
        <td style="border: transparent; text-align: left; padding-left: 10px;">
            ...
            <span style="font-size: 8px">
                        ...
                        Beneficiary's name:  {{$dispatcher->account_name}}
                      <br>
                        Beneficiary's Acc:  {{$dispatcher->account}}
                      <br>
                        SWIFT:  {{$dispatcher->swift}}
                      <br>
                        Beneficiary's Bank: {{$dispatcher->beneficiary_bank}}
                      <br>
                      <br>
                        Bank Address: {{$dispatcher->bank_address}}
                    </span>
...
<table style="height: 203px; width: 100%; border-collapse: collapse; font-size: 12px" border="1">
    ...
    <tr style="height: 38px;">
        ...
        <td style="width: 58.8796%; height: 38px; border-left: none; border-top: none; border-right: none; border-bottom-style: solid; border-bottom-color: black;"
            colspan="3"> 
            <strong> Account name: {{$customer->account_name}}</strong>
        ...
        <td style="width: 15.8562%; height: 38px; border-left: none; border-bottom: 1px solid black; border-right: 1px solid black;">
            @php
                $date_to = (clone $order->lead->start_date);

                $vehicle =  $order->lead->categories->first();
    ...
    <tr style="height: 36px;">
        ...
        <td style="width: 58.8796%; height: 36px; border-left: none; border-top: none; border-right: none; border-bottom-style: solid; border-bottom-color: black;"
            colspan="3">&nbsp;
            <strong> Swift:</strong> {{$customer->swift}}
        </td>
    ...
    <tr style="height: 36px;">
        ...
        <td style="width: 58.8796%; height: 36px; border-left: none; border-top: none; border-right: none; border-bottom-style: solid; border-bottom-color: black;"
            colspan="3"> 
            <strong>Beneficiary`s bank: </strong>{{$customer->beneficiary_bank}}.
            <strong>Code:</strong> {{$customer->code}}.
            <strong>Bank Address:</strong> {{$customer->bank_address}}
        </td>
        <td style="width: 2.60747%; height: 36px; border: none;"> </td>
...
<table class="two">
    ...
    @foreach($order->lead->categories as $vehicle)
        @php
...
<table class="three">
    ...
    <tr height="10px">
        ...
        <td>{{numfmt_format_currency($fmt, ($invoice->sum + $invoice->tax) / 100, $order->lead->domain->currency->code)}}</td>
    </tr>
    <tr height="10px">
