...
class Sms
{
    ...
    private $login = "Transbaza";        // логин клиента
    private $password =  '770b6bc56ae7b93e56c6c2bf256203a989953416';//'e4eaceeb49b6b6bb523cef72839c4a2052c5d3b0'; //""; // пароль
    private $sms_post = 0;                    // использовать метод POST
    private $sms_https = 0;                // использовать HTTPS протокол
    private $charset = "utf-8";    // кодировка сообщения: utf-8, koi8-r или windows-1251 (по умолчанию)
    private $debug = 0;                // флаг отладки
...
// Функция вызова запроса. Формирует URL и делает 5 попыток чтения через разные подключения к сервису
    ...
    {
        $url = $_url = ($this->sms_https ? "https" : "http") . "://smsc.ru/sys/$cmd.php?login=" . urlencode($this->login) . "&psw=" . urlencode($this->password) . "&fmt=1&charset=" . $this->charset . "&" . $arg;
...
// curl или fsockopen (только http) или включена опция allow_url_fopen для file_get_contents
    ...
    {
        $ret = "";
        $post = $this->sms_post || strlen($url) > 2000 || $files;
        ...
        if (function_exists("curl_init")) {
            ...
            if (!$this->sms_https && function_exists("fsockopen")) {
                $m = parse_url($url);
